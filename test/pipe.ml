let () =
  ...
  let _job =
    Iocp.write iocp ~file_offset:Optint.Int63.zero wfd buf ~off:0 ~len:5 `W ol
  in
  ...
  | Some t -> (
      ...
      let _job =
        Iocp.read iocp ~file_offset:Optint.Int63.zero rfd buf' ~off:0
          ~len:t.bytes_transferred `R ol
      in
      match Iocp.get_queued_completion_status iocp with
      | None -> assert false
      | Some t ->
          ...
          print_endline
            ("Received '" ^ Cstruct.to_string buf' ^ "' over the pipe");
