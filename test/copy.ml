...
let () =
  ...
  let out = Iocp.openfile iocp 1 Sys.argv.(2) [O_WRONLY; O_CREAT; O_TRUNC] 0o644 in
  let buf = Cstruct.create 1024 in
  let ol = Iocp.Overlapped.create ~off:Optint.Int63.zero () in
  let (_job : req Iocp.job option) = Iocp.read iocp ~file_offset:Optint.Int63.zero fd buf ~off:0 ~len:1024 `R ol in
  ...
  match Iocp.get_queued_completion_status iocp 1000 with
    ...
    | Some t ->
      ...
      let _job = Iocp.write iocp ~file_offset:Optint.Int63.zero out buf ~off:0 ~len:t.bytes_transferred `W ol in
      match Iocp.get_queued_completion_status iocp 1000 with
      | None -> assert false
      | Some t ->
